...
module.exports = (function() {
     ...
     */
    var Workbook = function(data, custSheetNames) {
        var self = this;
        ...
        if(data) {
            self.loadTemplate(data, custSheetNames);
        }
    };
    ...
    */
    Workbook.prototype.deleteSheet = function(sheetName){
      ...
      var match = /sheet(\d+)\.xml/.exec(sheet.filename)
      if (match) {
          sheetId = match[1];
      }

      Object.keys(self.archive.files).forEach(function(key){
        if (key.indexOf(sheetId) != -1)
     ...
     */
    Workbook.prototype.loadTemplate = function(data, custSheetNames) {
        var self = this;
        ...
        // create customized sheet names
        if (custSheetNames) {
            self.workbook.findall("sheets/sheet").forEach(x => {
                if (custSheetNames[x.attrib['name']]) {
                    x.attrib['name'] = custSheetNames[x.attrib['name']];
                }
            })
            self.archive.file(self.workbookPath, etree.tostring(self.workbook));
        }
    ...
    Workbook.prototype.substitute = function(sheetName, substitutions) {
        ...
        sheetData.findall("row").forEach(function(row) {
            ...
            row.findall("c").forEach(function(cell) {
                ...
                if(cell.attrib.t === "s") {
                    ...
                    var cellValue   = cell.find("v"),
                        ...
                        column      = cell.attrib.r.substring(0, 1);
                    ...
                    self.extractPlaceholders(string).forEach(function(placeholder) {
                        ...
                        // Only allow if blocks in first cells
                        if(column === 'A') {
                            if(!hideBlock && placeholder.full && placeholder.type === "if") {
                                appendCell = false;
                                hideBlock = substitution ? false : placeholder.name;
                                self.pushUp(self.workbook, sheet.root, namedTables, currentRow, 1);
                                if (!hideBlock) {
                                    --totalRowsInserted;
                                }
                                return;
                            } else if(placeholder.full && placeholder.type === "endif") {
                                appendCell = false;
                                if (hideBlock === placeholder.name) {
                                    hideBlock = false;
                                }
                                self.pushUp(self.workbook, sheet.root, namedTables, currentRow, 1);
                                --totalRowsInserted;
            ...
            if (hideBlock) {
                self.pushDown(self.workbook, sheet.root, namedTables, currentRow, 1);
                --totalRowsInserted;
                return;
            }
    ...
    // `currentRow` and push up by `numRows` (used when rows are removed).
    Workbook.prototype.pushUp = function(workbook, sheet, tables, currentRow, numRows) {
        var self = this;
        ...
        sheet.findall("mergeCells/mergeCell").forEach(function(mergeCell) {
            ...
            if(mergeStart.row > (currentRow + numRows)) {
        ...
        tables.forEach(function(table) {
            ...
            if(tableStart.row > (currentRow + numRows)) {
        ...
        workbook.findall("definedNames/definedName").forEach(function(name) {
            ...
            if(self.isRange(ref)) {
                ...
                if(namedStart.row > (currentRow + numRows)) {
                ...
                if(namedRef.row > (currentRow + numRows)) {
